...
class NERModel(BaseModel):
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("chars"):
            if self.config.use_char_lstm:
                ...
                # bi lstm on chars
                cell_fw = rnn.LSTMCell(self.config.hidden_size_char,initializer=tf.glorot_uniform_initializer())
                cell_bw = rnn.LSTMCell(self.config.hidden_size_char,initializer=tf.glorot_uniform_initializer())
            ...
            if self.config.use_char_cnn:
                ...
                with tf.variable_scope("conv"):
                        weights = tf.get_variable(name="weights", shape=[self.config.filter_size, self.config.dim_char, 1, self.config.filter_deep],
                                                  initializer=tf.glorot_uniform_initializer())
                        biases = tf.get_variable(name="biases", shape=[self.config.filter_deep],initializer=tf.constant_initializer(0))
                        conv = tf.nn.conv2d(char_embeddings, weights, strides=[1, 1, 1, 1], padding='VALID',name="conv")
                        relu = tf.nn.relu(tf.nn.bias_add(conv, biases))
                        pool = tf.nn.max_pool(relu, ksize=[1, self.max_word_lengths - self.config.filter_size + 1, 1, 1],strides=[1, 1, 1, 1], padding="VALID", name="pool")
                pool_flatten = tf.reshape(pool, [s[0], s[1], self.config.filter_deep])
                self.char_embeddings = pool_flatten
        word_embeddings = tf.concat([word_embeddings, self.char_embeddings], -1)
        print(word_embeddings.get_shape())
        self.word_embeddings = tf.nn.dropout(word_embeddings, self.dropout)

    def add_logits_op(self):
        with tf.variable_scope("bi-lstm"):
            cell_fw =rnn.LSTMCell(self.config.hidden_size_lstm,initializer=tf.glorot_uniform_initializer())
            cell_bw = rnn.LSTMCell(self.config.hidden_size_lstm,initializer=tf.glorot_uniform_initializer())
    ...
    def run_predict(self, test,save_path):
